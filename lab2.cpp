...
void BigInt::read(const std::string& file_name) {
...
BigInt BigInt::operator+(const BigInt& bigInt) const {
	...
	if (isAddOp) {
		...
		BigInt en = isNeg ? (bigInt - (-BigInt(*this))) : (*this - (-BigInt(bigInt)));
...
BigInt BigInt::operator-(const BigInt& bigInt) const {
	...
	if (!isNeg && !bigInt.getIsNeg()) {
		...
		BigInt en = isNeg && bigInt.getIsNeg() ? (-BigInt(bigInt) - (-BigInt(*this))) : (*this + -BigInt(bigInt));
		return en;
	}
}
